...
use crate::common;
use crate::common::{Component, ComponentResult, extract_component_2, render_component_bindings};
...
pub fn on_event(attr: TokenStream, item: TokenStream, event_type: EventType) -> Result<TokenStream> {
    let (
        module_path,
        function,
        ctx,
    ...
    let normalized_function = generate_normalized_function(&function, (ctx, ctx_ident), &components, event_type);

    let registration = generate_registration(module_path, &function, signal_type, &components, event_type);
...
fn extract_input(attr: TokenStream, item: TokenStream, event_type: EventType) -> Result<(ExprPath, ItemFn, PatType, Option<Type>, Vec<Component>, Ident)> {
    ...
    let args: Vec<_> = function.sig.inputs.iter()
        .map(|it| {
            match it {
                FnArg::Receiver(_) => panic!("{} is applicable only to top-level free functions", attribute_name),
                FnArg::Typed(it) => it
            }
        })
        .collect();
    if args.is_empty() {
        panic!("{} requires first argument - {}", attribute_name, ctx_type_name);
    }
    let ctx = *args.first().unwrap();

    let signal_type = extract_signal_type_and_validate_ctx(ctx, event_type)
        .map_err(|ctx_type_error| {
            Error::new(
                ctx.span(),
                format!("{} first parameter should be {} value (error_code: {})", attribute_name, ctx_type_name, ctx_type_error),
            )
        })?;
    ...
    if let EventType::OnSignalGlobal = event_type {
        if components.is_empty().not() {
            return Err(Error::new(Span::call_site(), format!("{} doesn't accept components", attribute_name)));
        }
    }
    ...
    Ok((module_path, function.clone(), ctx.clone(), signal_type, components, ctx_ident.clone()))
}

fn extract_signal_type_and_validate_ctx(ctx: &PatType, event_type: EventType) -> std::result::Result<Option<Type>, &'static str> {
    let signal_type = match ctx.ty.deref() {
        Type::Path(it) => it,
        _ => { return Err("invalid ctx type"); }
    };
    ...
    let args = match &ident.arguments {
        ...
        _ => { return Err("invalid ctx generic type"); }
    };
...
fn generate_normalized_function(function: &ItemFn, (ctx, ctx_ident): (PatType, Ident), components: &Vec<Component>, event_type: EventType) -> ItemFn {
    let mut normalized_function = function.clone();
    normalized_function.sig.inputs = Punctuated::new();
    normalized_function.sig.inputs.push(FnArg::Typed(ctx.clone()));
    ...
    normalized_function.sig.inputs.push(FnArg::Typed(entity_arg));

    render_component_bindings(&ctx_ident, &components, &mut normalized_function.block, &entity_ident);
    normalized_function
}

fn generate_registration(module_path: ExprPath, function: &ItemFn, signal_type: Option<Type>, components: &Vec<Component>, event_type: EventType) -> ItemFn {
    ...
    let registration = match event_type {
        EventType::OnSignal => quote! { registry.add_entity_signal_handler::<#signal_type>(filter_key, #function_name); },
        EventType::OnSignalGlobal => quote! { registry.add_global_signal_handler::<#signal_type>(#function_name); },
        EventType::OnAppear => quote! { registry.add_entity_appear_handler(filter_key, #function_name); },
        EventType::OnDisappear => quote! { registry.add_entity_disappear_handler(filter_key, #function_name); },
    };
    parse_quote! {
        #[ctor::ctor]
