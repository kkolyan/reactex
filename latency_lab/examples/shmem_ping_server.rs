...
fn main() {
    remove_file("shmem_ping_server_input");
    remove_file("shmem_ping_server_output");
    ...
    loop {
        let ping_message = shmem_ping_receive(&receiver, None);
        shmem_ping_send(&ping_message, &sender, None);
    }
}